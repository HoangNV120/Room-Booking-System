...
public class HotelService
{
    ...
    public HotelService(HotelBookingDBContext context, IMapper mapper, ImageService imageService)
    {
        _context = context;
        _mapper = mapper;
    ...
    public async Task<HotelResponseDTO> CreateHotel(HotelCreateDTO request)
    {
        ...
        {
            var imageName = $"hotel_{hotel.Id}";
            hotel.ImageUrl = await _imageService.UploadImageAsync(request.Image, null, imageName);
        }
    ...
    public async Task<HotelResponseDTO> UpdateHotel(Guid id, HotelUpdateDTO request)
    {
        ...
        if (request.Image != null && request.Image.Length > 0)
        {
            var imageName = $"hotel_{id}";
            hotel.ImageUrl = await _imageService.UploadImageAsync(request.Image, hotel.ImageUrl, imageName);
        }
